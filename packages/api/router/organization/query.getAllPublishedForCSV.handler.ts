...
import { type TGetAllPublishedForCSVSchema } from './query.getAllPublishedForCSV.schema'
...
const getAllPublishedForCSV = async ({ input }: TRPCHandlerParams<TGetAllPublishedForCSVSchema>) => {
	...
	const results = await prisma.$queryRaw<OrganizationCsvExportRow[]>(
      ...
      WHERE
          published = TRUE AND deleted = FALSE
      ORDER BY
...
export default getAllPublishedForCSV
