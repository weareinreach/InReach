...
| Content Search & Bulk Edit                                     | Big                            | No existing keyword search over `description`/service-name text (only org-name trigram search exists today) — needs new search indexing before any query can be built, plus a new page/nav entry, a new `dataPortalManager`+ permission tier, and a not-yet-designed guided-review workflow for anything beyond simple inline edits. See [`Organizations/content-search-bulk-edit.md`](../Organizations/content-search-bulk-edit.md).                                                                                       |
...
Search across organization and service names, descriptions, and attributes by keyword and/or
...
needs sensitive population-language review. Gated `dataPortalManager`+, proposed as a new side-nav
page rather than a mode of the existing Organization table. MVP scope is inline edit of matched
records (no true batch mutation); a guided before/after review step for broader changes is a later
phase, not yet designed. Full design discussion, open questions, and sizing rationale moved to
[`Organizations/content-search-bulk-edit.md`](../Organizations/content-search-bulk-edit.md).
