...
decoded AS (
  SELECT
    ...
    (
      SELECT
        to_timestamp((inner_query.ts / (1000.0) :: double precision)) AS to_timestamp
      FROM
        (
          SELECT
            (
                  ...
                  (
                    (
                      (
                        POSITION(
                          (
                            "substring"(split_part(parsed.id, '_' :: text, 2), i.i, 1)
                          ) IN ('0123456789ABCDEFGHJKMNPQRSTVWXYZ' :: text)
                        ) - 1
                      )
                    ) :: double precision * (
                      (32) :: double precision ^ ((10 - i.i)) :: double precision
                    )
                  )
                ) AS sum
              FROM
                generate_series(1, 10) i(i)
            ) AS ts
        ) inner_query
    ) AS ulid_timestamp,
    EXTRACT(
