	...
	forContactInfo: [
		{
			...
			description: {
				...
				defaultText: 'Schedule An Appointment',
			},
		},
		{
			id: 'oeml_01GVH3VEVDX7QVQ4QA4C1XXVN3',
		...
		},
		{
			...
			description: {
				...
				defaultText: 'Youth Mental Health Services',
			},
		},
	],
} satisfies MockDataObject<'orgEmail'>
export const orgEmail = {
	get: getTRPCMock({
		path: ['orgEmail', 'get'],
		response: orgEmailData.get,
	}),
	...
	forContactInfo: getTRPCMock({
		...
		type: 'query',
		response: ({ locationOnly, serviceOnly }) => {
			if (locationOnly !== undefined && serviceOnly !== undefined) {
				return orgEmailData.forContactInfo.filter(
					(record) => record.locationOnly === locationOnly && record.serviceOnly === serviceOnly
				)
			}
			if (locationOnly !== undefined) {
				return orgEmailData.forContactInfo.filter((record) => record.locationOnly === locationOnly)
			}
			if (serviceOnly !== undefined) {
				return orgEmailData.forContactInfo.filter((record) => record.serviceOnly === serviceOnly)
			}
			return orgEmailData.forContactInfo
		},
	}),
