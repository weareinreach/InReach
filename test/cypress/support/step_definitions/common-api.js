import {Then, Given, When, defineParameterType} from 'cypress-cucumber-preprocessor/steps';
...
Then(`response body value of key {string} matches fixture {string} with key {string}`,(key,fixture,key2)=>{
  cy.get('@response').then(body=>{
    cy.fixture(fixture).then(fixtureJson=>{
      expect(key.split('.').reduce((o,i)=>o[i], body)).to.eq(key2.split('.').reduce((o,i)=>o[i], fixtureJson));
    });
  });
});
