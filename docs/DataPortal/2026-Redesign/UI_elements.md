...
- **Toolbar extension slot** — `OrganizationTable` passes a `toolbarExtra` node into `DataTable`. As of this pass it holds two labeled dropdown quick-filters (Publish Status, Create Method — see item 4 below, now built for Organizations) plus the Deleted icon toggle. The mechanism for adding table-specific controls into a shared toolbar already exists and is in active use; it isn't yet extended to other tables (e.g. Data entry teams on Users).
...
4. **Toolbar quick-filters.** ~~Extend the existing icon-toggle-only toolbar~~ — **done for Organizations**: Publish Status and Create Method are now labeled `Select` dropdowns in the toolbar (Deleted remains an icon toggle). Still to do: Service type on Organizations, Data entry teams on Users, and generalizing the pattern so it's configured per table rather than hardcoded per component.
