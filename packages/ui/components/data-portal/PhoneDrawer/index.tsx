...
import { Drawer, Group, LoadingOverlay, Modal, Stack, Text, Title } from '@mantine/core'
...
export const PhoneDrawer = ({ opened, onClose, id, createNew = false }: PhoneDrawerProps) => {
	...
	const siteUpdate = api.orgPhone.upsert.useMutation({
		onSettled: (data, error, variables) => {
			...
			// reaching the list.
			apiUtils.orgPhone.forEditDrawer.invalidate({ id: phoneId, orgId: orgId ?? '' }, { refetchType: 'none' })
	...
	useEffect(() => {
		// This drawer is now a single shared instance reused across every row plus the "Create
		// new" trigger, rather than one dedicated instance per row/action - so a session's leftover
		// form state (a different phone's data, or an abandoned create-new draft) would otherwise
		// still be sitting in the form the next time it opens. Resetting on the closed-to-open
		// transition clears that out before this session's own data (if any) loads in via `values`
		// above; `isSaved` is reset here too so a previous session's "Saved" checkmark doesn't
		// linger onto an unrelated new session.
		if (opened && !wasOpened.current) {
			...
			reset({
				...
				published: true,
				deleted: false,
				...(createNew && hasLocationId !== null ? { linkLocationId: hasLocationId } : {}),
			})
		}
		wasOpened.current = opened
	}, [opened, createNew, phoneId, hasLocationId, reset])
	useEffect(() => {
	...
	}, [onClose, formIsDirty, modalHandler])
	...
	const handleCloseNoSave = useCallback(() => {
		...
		modalHandler.close()
		onClose()
	}, [onClose, modalHandler, reset])

	return (
		<Drawer.Root onClose={handleClose} opened={opened} position='right' zIndex={10001} keepMounted>
			<Drawer.Overlay />
