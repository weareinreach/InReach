...
import { CreateNote, GetNoteByRecord } from '~api/schemas/internalNote'

export const internalNoteRouter = defineRouter({
	byId: staffProcedure
		.meta({ hasPerm: 'internalNotesRead' })
		.input(z.string())
		.query(async ({ ctx, input }) => ctx.prisma.internalNote.findFirstOrThrow({ where: { id: input } })),
	getAllForRecord: staffProcedure
		.meta({ hasPerm: 'internalNotesRead' })
		.input(GetNoteByRecord)
		.query(async ({ ctx, input }) => ctx.prisma.internalNote.findMany({ where: input })),
