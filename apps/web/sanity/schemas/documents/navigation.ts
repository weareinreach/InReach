...
import { SlugParent, defineField, defineType } from 'sanity'
...
const getLabel = (parent: NavSlugParent) => parent.label.filter((x) => x._key === 'en-US')[0].value
			...
			of: [
				{
					...
					preview: {
						...
						prepare(value) {
							...
							const itemTitle = subItem.filter((item) => item._key === 'en-US')[0].value
							return {
								title: `${itemTitle ?? ''}`,
