...
1. **Data-Portal header bar.** A new, dedicated persistent bar for the Data Portal specifically — distinct from the app-wide consumer `Navbar` (logo/avatar-menu/"Safety Exit"), which currently wraps every page unconditionally, including `/admin`, and has nothing to do with the Data Portal. The new bar shows top-level sections (Organizations / Tasks / Admin) with the active one indicated, and the signed-in user's name as visible text next to their avatar — today only an avatar icon renders anywhere, never the name.
...
| [`packages/ui/components/sections/Navbar.tsx`](../../../packages/ui/components/sections/Navbar.tsx)                                                                                                                   | The consumer-facing header this project's new Data-Portal header bar is explicitly _not_ an extension of                            |
