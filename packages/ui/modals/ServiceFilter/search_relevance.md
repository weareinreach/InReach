...
Build an **Empowered Search** engine that transitions from rigid "yes/no" filtering to a flexible 'smart' system. The goal is to ensure users in high-stress situations can find the most relevant resources without encountering "no results" dead ends.
...
3.  **Service Match Count**: In "Match Any", organizations providing _more_ of the requested services rank higher.
4.  **Hierarchical Match Logic**: Parameters for intra-group and inter-group logic (AND/OR). Default is AND but OR will be used if the results are 0 or a few
5.  **National/Remote Inclusion**: A toggle to include non-local or remote resources, which appears at the top by default.
...
1.  **Sidebar Updates**:
    - Remove 'Coming Soon' mask
      ...
      - enable darg and drop to set Sort order
      - Check Box to Show/Hide remote/online options (Default is to Show)

### 3.2. Phase 2 (Near term Needs)

1.  **Search Updates**:
    - Distance Slider
      - National/Remote checkbox
...
### 4.2. Explicit Search Trigger (to be reviewed)
...
1. **Frontend**: Add Advanced Toggle, update the SearchSidebar component
2. **Backend**: Create V2 API handlers, schema, and queries
3. **Schema**: Define V2 Zod inputs for distance defaults and sort biases
...
> - In the case where there are 0 or very few (less than 5) results. The system will change the search to look for any Organzations the have Food Assistance, Abortion Services, OR any other free services. Then when the results are provided a message will appear informing the user "Hey we could not find Free Food Assistnace or Free Abortion services, but these organizations may still be able to help."
